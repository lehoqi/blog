...
- 2026-05-31 voice/map/animation re-check: with a long-running `speechSynthesis` test double, submitting an answer before narration ended called `cancel()` immediately; the old question did not restart after submit; the current equipped ambulance theme rendered `急救救援`/`🏥`/`🚑`/`🦠`; non-final rush showed particles and advanced the map from dot 1 to dot 2; final Boss finisher appeared before the single `5 / 5` result page.
